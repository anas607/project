...
import {
  BaseUrl,
  show_import_internal_mails,
  show_internal_mails_export,
} from "../../API/api";
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const realData = Array.isArray(response.data?.[0])
          ? response.data[0]
          : response.data;
          ...
          <Table sx={{ width: "1573px", height: "88px" }}>
            <TableHead sx={{ width: "1573px", height: "88px" }}>
              <TableRow sx={{ backgroundColor: "rgb(14, 74, 35)" }}>
                ...
                <TableCell align="center" sx={{ color: "white", py: 1.5 }} />
              </TableRow>
            </TableHead>

            {/* كل الحالات لازم تكون داخل TableBody */}
            <TableBody>
              {error ? (
                <TableRow>
                ...
                <TableRow>
                  <TableCell colSpan={8} align="center">
                    {/* إذا Loading بيرجع <h2> أو <p> عادي لأنه صار داخل <td> */}
                    <Loading />
                  </TableCell>
                </TableRow>
              ) : rows.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={8} align="center">
                    {/* لازم NoData ما ترجع <tr> لوحدها. خليه عنصر عادي (div/Box) */}
                    <NoData />
                  </TableCell>
                </TableRow>
              ) : (
