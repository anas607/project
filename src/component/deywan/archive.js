...
export default function Archiv() {
  ...
  const mailTypes = [
    "البريد الصادر الخارجي",
    "البريد الداخلي الوارد",
    "البريد الداخلي الصادر",
  ];
      ...
      try {
        ...
        if (selectedType === "البريد الصادر الخارجي") {
          response = await getData(
            `${BaseUrl}${TRANSACTION}${ARCHIVED_EXPORT}`
          );
          const realData = Array.isArray(response.data?.[0])
            ? response.data[0]
            : response.data;
        ...
        if (selectedType === "البريد الداخلي الوارد") {
          response = await getData(
            `${BaseUrl}${ARCHIV}${IMPORT}${INTERNAL}${MAILS}`
          );
          const realData = Array.isArray(response.data?.[0])
            ? response.data[0]
            : response.data;
  ...
  return (
    <>
      ...
      >
        ...
        {<EnternalMails open={setenter} onclose={() => setOpenEnter(false)} />}
      </Box>
    </>
  );
}
